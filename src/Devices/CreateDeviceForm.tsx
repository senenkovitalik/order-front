...
import type { CreateDeviceFormProps, DeviceInputs } from "./types";
...
export default function CreateDeviceForm({
  ...
  const onSubmit: SubmitHandler<DeviceInputs> = (data) => onCreate(data);
  ...
  const {
    ...
    VpnProfilesWithoutDevicesQueryVariables
  >(VPN_PROFILES_WITHOUT_DEVICE_QUERY, {
    variables: { filter: { hasDevice: false } },
  });

  const employees = employeesData?.employees ?? [];
  const vpnProfiles = vpnProfilesData?.vpnProfiles ?? [];
  ...
  return (
    ...
    >
      <Container>
        ...
        <select
          className="w-full py-3 px-5 my-2 mx-0 inline-block border border-solid outline-[#ccc] box-border"
          defaultValue={employees.length > 0 ? employees[0].id : ""}
